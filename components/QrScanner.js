...
export const QrScanner = ({ onScan, onClose }) => {
  ...
  }, []); // Removed onScan from dependencies to prevent camera restart
  ...
  return (
    React.createElement('div', { className: "relative w-full aspect-square bg-slate-900 rounded-md overflow-hidden flex items-center justify-center" },
      loading && !error && React.createElement(Spinner, { className: "w-10 h-10 text-sky-500", title: "Загрузка камеры..."}),
      error && React.createElement('p', { className: "text-red-400 p-4 text-center" }, error),
      React.createElement('video', { ref: videoRef, className: `w-full h-full object-cover ${loading || error ? 'hidden' : ''}` }),
      React.createElement('canvas', { ref: canvasRef, className: "hidden" }),
      React.createElement('div', { className: "absolute inset-0 border-8 border-white/20 rounded-md", style: { clipPath: 'polygon(0% 0%, 0% 100%, 25% 100%, 25% 25%, 75% 25%, 75% 75%, 25% 75%, 25% 100%, 100% 100%, 100% 0%)' }})
    )
  );
};
