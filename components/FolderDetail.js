    ...
    const getStatusIndicator = (status) => {
        switch (status) {
            case PeerConnectionStatus.Connected:
                return React.createElement('span', { className: "w-3 h-3 rounded-full bg-green-500", title: "Подключено" });
            case PeerConnectionStatus.Connecting:
                return React.createElement(Spinner, { className: "w-4 h-4 text-sky-400", title: "Подключение..."});
            case PeerConnectionStatus.Disconnected:
                return React.createElement('span', { className: "w-3 h-3 rounded-full bg-slate-500", title: "Отключено" });
            case PeerConnectionStatus.Failed:
                return React.createElement('span', { className: "w-3 h-3 rounded-full bg-red-500", title: "Ошибка" });
        }
    };
                    ...
                    folder.peers.map((peer) => (
                        React.createElement('li', { key: peer.id, className: "flex items-center justify-between py-3 px-2" },
                            React.createElement('div', { className: "flex items-center" },
                                React.createElement('div', { className: "w-4 h-4 flex items-center justify-center mr-4 flex-shrink-0" }, getStatusIndicator(peer.connectionStatus)),
